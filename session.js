...
const speakerAndBoothList = function (values, isPreview) {
  if (values?.speakers?.length || values?.booths?.length || !values?.sessionsLibrary?.selected?.id) {
    if (isPreview) {
      return `
  <tr>
    <td>
      <p id="${values?.sessionsLibrary?.selected?.id}-sessionSpeaker">${
        !values?.sessionsLibrary?.selected?.id
          ? ['a', 's', 'd'].toString()
          : values?.speakers?.toString()
      }</p>
      <p id="${values?.sessionsLibrary?.selected?.id}-sessionBooth">${
        !values?.sessionsLibrary?.selected?.id
          ? ['q', 'w', 'e'].toString()
...
const toolEmailTemplate = function (values, isViewer = false) {
  return `
    <table sessionId="${
      values?.sessionsLibrary?.selected?.id ? values?.sessionsLibrary?.selected?.id : ''
