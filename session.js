...
const speakerAndBoothList = function (values, isPreview) {
  if (values?.speakers?.length || values?.booths?.length || !values?.booth1Library?.selected?.id) {
    if (isPreview) {
      return `
  <tr>
    <td>
      <p id="${values?.booth1Library?.selected?.id}-sessionSpeaker">${
        !values?.booth1Library?.selected?.id
          ? ['a', 's', 'd'].toString()
          : values?.speakers?.toString()
      }</p>
      <p id="${values?.booth1Library?.selected?.id}-sessionBooth">${
        !values?.booth1Library?.selected?.id
          ? ['q', 'w', 'e'].toString()
...
const toolEmailTemplate = function (values, isViewer = false) {
  return `
    <table sessionId="${
      values?.booth1Library?.selected?.id ? values?.booth1Library?.selected?.id : ''
