...
<% _.forEach(products, function(item) { %>
  ...
  <img src="<%= item.profile_img %>" style="max-height: 150px;min-height: 100px;width: 100%;" />
    <h4 style="margin: 8px 0; text-align: left; color: ${values.speakerTitleColor};"><%= item.name %></h4>
    <h5 style="margin: 8px 0; text-align: left; color: ${values.speakerDesignationCompanyColor};"><%= item.designation %>,<%= item.company %> </h5>
  </div>
<% }); %>
`);
