...
const boothItemsTemplate = _.template(`<% _.forEach(booths, function(item) { %>
  ...
  <div class="booth-media"> <img src="<%= item.profile_img %>" alt="booth image" style="max-height: 90px;width: 100%; object-fit: contain;border-radius:8px" /> </div>
...
const toolTemplate = function (values, isViewer = false) {
  ...
  return `<div class="booth-card card" style="position:relative;background-color:${values.boothBGColor}">
    <div class="booth-img"> <img src="${values?.boothImage?.url}" alt="booth image" style="width: 100%; object-fit: contain; border-radius:8px" />
    </div>
...
const toolEmailTemplate = function (values, isViewer = false) {
  ...
  <div boothId="${values?.boothLibrary?.selected?.id}" style="position:relative;background-color:${values.boothBGColor};margin-bottom: 15px;padding:18px 10px;border-radius: 8px;">
    <div style="border-radius: .8rem; border: 1px solid #f1f1f1;height: 85px;width: 85px;margin: auto; display: flex;background-color: #fff;"> <img id="${values?.boothLibrary?.selected?.id}-boothImg" src="${values?.boothImage?.url}" alt="booth image" style="width: 100%; object-fit: contain;border-radius:8px" />
    </div>
